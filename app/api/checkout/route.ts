...
export async function POST(req: NextRequest) {
  ...
  if (!purchaseId) {
    // Fee waivers: Early Adopters (first 100 accounts) pay 0% for life; everyone
    // else gets 0% for their first 30 days, then the flat platform fee.
    const { data: sellerRaw } = await supabase
      .from('profiles')
      .select('created_at, early_adopter')
      .eq('id', repo.owner_id)
      .maybeSingle()
    const seller = sellerRaw as { created_at: string; early_adopter: boolean } | null
    const isEarlyAdopter = seller?.early_adopter === true
    const inFirstMonth = seller?.created_at
      ? Date.now() - new Date(seller.created_at).getTime() < 30 * 86_400_000
      : false
    const feeCents = isEarlyAdopter || inFirstMonth ? 0 : Math.round(repo.price_cents * PLATFORM_FEE)
